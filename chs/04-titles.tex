...
\cmd{\thanks} 명령어는 \Icn{footnote} 카운터를 사용하며, 표제 이후에는 각주
표지\tidx{footnote!mark,각주!표지}를 1부터 시작하기 위해 영으로 초기화된다.
...
\cmd{\thanksheadextra} 명령어는 \meta{pre}와 \meta{post}를 각각 표제 영역에
있는 감사의 말 전후에 삽입할 것이다.
기본적으로 \meta{pre}와 \meta{post}는 비어있다.
...
\cmd{\thanksmark}\marg{n} 명령어는 \cmd{\thanks} 명령어의 \meta{n}번째 표지와
같은 표지를 출력한다는 점에서 \cmd{\footnotemark}\oarg{n}와 유사하다.
각주\tidx{footnote,각주}의 감사의 말에는 어떠한 변경도 생기지 않는다.
...
각주\tidx{footnote,각주}의 감사의 말 표지는 너비 \lnc{\thanksmarkwidth}의
박스에 오른쪽 정렬되어 조판된다.
...
% The code for this is roughly:
  각주\tidx{footnote,각주} 위치의 감사의 말 표지는 \cmd{\thanksfootmark}로
  식자된다.
  이에 대한 코드는 대략 다음과 같다.
...
여러분은 \cmd{\makethanksmarkhook}가 유용한 일을 하도록 재정의할 수 있다.
...
베이스라인 간격을 나타낸다.
이 예시에서 8pt는 10pt 문서의 보통 \cmd{footnotesize}이며, 11pt는 11pt 문서에서
...
기본적으로 \Ie{titlingpage} 환경에서 \cmd{\thanks} 문구 위에는 선이 없다.
만약 해당 환경에서 선을 그리고 싶다면 \cmd{\usethanksrule}을 \cmd{\maketitle}
명령 전에 넣어 \cmd{\thanksrule}의 현재 정의에 맞게 선을 출력하도록 할 수 있다.
\cmd{\thanksrule}은 preamble\index{preamble} 끝에 정의된 \cmd{\footnoterule}의
사본으로 초기 설정되어 있다.
\cmd{\thanksrule}의 정의는 \verb|\begin{document}| 이후에 변경할 수 있다.
만약 \cmd{\thanksrule}의 정의가 변경된 후 \cmd{\usethanksrule}이 사용되었다면,
각주\index{footnote,각주}에 대해서도 동일한 선이 사용될 수 있다.
...
각주\tidx{footnote,각주}와 감사의 말 주석의 세로 위치에 대한 매개변수와 기본
\cmd{\footnoterule}은 같다(\pref{fig:fn}의 \fref{fig:fn}을 보라.).
각주\tidx{footnote,각주}와 감사의 말 주석의 세로 간격이 달라야 한다면 이들 중
일부를 바꿔야 한다.
