...
사이에 선이 그려질 수 있으며, 비슷하게 조판 영역\tidx{typeblock,조판 영역}
아래와 바닥글\tidx{footer,바닥글} 위에 선이 그려질 수 있다.
\cmd{\makeheadrule} 매크로는 \meta{style} 페이지 양식
머릿글\tidx{header,머릿글} 아래 선의  \meta{width}와 \meta{thickness}를
지정한다.
추가적 인자 \meta{skip}은 바닥 선의 세로 위치를 지정하는 길이이다
(\cmd{\footruleskip}을 보라).
\cmd{\makepagestyle} 매크로는 \meta{width}를 \lnc{\textwidth}로 초기화하고
\meta{thickness}를 0pt로 초기화해서, 기본적으로는 선이 보이지 않는다.
...
% option.
\lnc{\normalrulethickness}는 보이는 선의 기본\tidx{rule!thickness,선!두께}
두께로, 0.4pt이다.
...
여백\tidx{margin!outer,여백!바깥}으로 삐져 나와 있고, 아래에 선이 있다.
...
그리고 머릿글\tidx{header,머릿글} 선의 너비와 두께를 설정해야 보일 것이다.
\begin{lcode}
